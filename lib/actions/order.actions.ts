...
export async function createOrder() {
    try {
        ...
        if (!cart || cart.items.length === 0) {
            return { sucess: false, message: 'Your cart is empty', redirectTo: '/cart'}
        }

        if (!user.address) {
            return { sucess: false, message: 'No shipping address', redirectTo: '/shipping-address'}
        }

        if (!user.paymentMethod) {
            return { sucess: false, message: 'No payment method', redirectTo: '/payment-method'}
        }
